...
def get_blocks(model):
    ...
    if model.__class__.__name__ in ("LlamaForCausalLM", "Qwen2ForCausalLM") or model.__class__.__name__ == "DreamModel":
...
def move_embed(model, device):
    if isinstance(model, (LlamaForCausalLM, Qwen2ForCausalLM)) or model.__class__.__name__ == "DreamModel":
        model.model.embed_tokens = model.model.embed_tokens.to(device)
        model.model.rotary_emb = model.model.rotary_emb.to(device)
    elif model.__class__.__name__ == 'LLaDAModelLM':
        model.model.transformer.wte = model.model.transformer.wte.to(device)
        # model.model.rotary_emb = model.model.rotary_emb.to(device)
