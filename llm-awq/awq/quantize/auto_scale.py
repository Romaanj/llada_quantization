...
from .modeling_llada import LLaDALlamaBlock
...
def auto_scale_block(module, module_kwargs, w_bit, q_config, input_feat):
    ...
    if isinstance(module, OPTDecoderLayer):
    ...
    elif isinstance(module, (LlamaDecoderLayer, Qwen2DecoderLayer)) or module.__class__.__name__ == "DreamDecoderLayer":
        # attention input
        scales_list.append(
            _auto_get_scale(
    ...
    elif module.__class__.__name__ == 'LLaDALlamaBlock':
        # attention input
        scales_list.append(
            _auto_get_scale(
                prev_op=module.attn_norm,
                layers=[
                    module.q_proj,
                    module.k_proj,
                    module.v_proj,
                ],
                inp=input_feat["q_proj"],
        ...
        # attn out
        # Please refer to https://github.com/mit-han-lab/llm-awq/pull/67#issue-1850622696
        if module.v_proj.weight.shape == module.attn_out.weight.shape:
            scales_list.append(
                _auto_get_scale(
                    prev_op=module.v_proj,
                    layers=[module.attn_out],
                    inp=input_feat["attn_out"],
                )
            )
        # fc1
        scales_list.append(
            _auto_get_scale(
                prev_op=module.ff_norm,
                layers=[module.ff_proj, module.up_proj],
                inp=input_feat["ff_proj"],
                module2inspect=module,
            )
        )
        # fc2
        scales_list.append(
            _auto_get_scale(
                prev_op=module.up_proj,
                layers=[module.ff_out],
                inp=input_feat["ff_out"],
            )
        )
...
def apply_scale(module, scales_list, input_feat_dict=None):
    for prev_op_name, layer_names, scales in scales_list:
        ...
        elif isinstance(prev_op, (nn.LayerNorm, LlamaRMSNorm, Qwen2RMSNorm)) or 'RMSLayerNorm' in prev_op.__class__.__name__ or prev_op.__class__.__name__ == 'DreamRMSNorm':
