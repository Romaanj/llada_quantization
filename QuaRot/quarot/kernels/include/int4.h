...
using Int4Subbyte = cutlass::MySubbyteReference<cutlass::int4b_t, uint8_t>; // 在 CUTLASS 中，MySubbyteReference<T, StorageType> 是一个“位级引用”工具类，用于 在一个字节数组中抽取/写入低精度（如 int4）数据位段。
using Int4Storage = Int4Subbyte::Storage; // Int4Subbyte::Storage 表示 MySubbyteReference<...> 内部定义的一个嵌套类型 Storage。它通常表示该引用操作的底层存储类型，即实际用于存放多个 int4 值的容器类型。
