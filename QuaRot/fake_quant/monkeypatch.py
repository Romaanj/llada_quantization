...
def add_wrapper_after_function_call_in_module(module, function_name, wrapper_fn):
    '''
    This function adds a wrapper after the output of a function call in the module.
    Only calls directly in the module are affected. Calls by other functions called in the module are not affected.
    '''
    func = getattr(module, function_name, None)

    if func is None:
        raise AttributeError(f'{module} has not attribute {function_name}')
    wrapper = wrapper_fn(func)
