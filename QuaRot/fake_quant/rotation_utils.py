...
def fuse_layer_norms(model):
    ...
    for layer in layers:
        ...
        if model_type == model_utils.LLAMA_MODEL or 'dream' in model.__class__.__name__.lower():
            ...
            fuse_ln_linear(layer.attn_norm, [layer.q_proj, layer.k_proj, layer.v_proj])
            fuse_ln_linear(layer.ff_norm, [layer.up_proj, layer.ff_proj])
        else:
    ...
    if 'llada' in model_type.__name__.lower():
        norm_type = model.model.transformer.ln_f.__class__
    elif 'dream' in model_type.__name__.lower():
        norm_type = model.model.norm.__class__
...
def rotate_attention_inputs(layer, Q, model_type) -> None:
    ...
    if 'llada' in layer.__class__.__name__.lower():
        for W in [layer.q_proj, layer.k_proj, layer.v_proj]:
...
def rotate_attention_output(layer, Q, model_type) -> None:
    ...
    if model_type == model_utils.LLAMA_MODEL or 'dream' in model_type.__name__.lower():
...
def rotate_mlp_input(layer, Q, model_type):
    ...
    if model_type == model_utils.LLAMA_MODEL or 'dream' in model_type.__name__.lower():
        ...
        mlp_inputs = [layer.up_proj, layer.ff_proj]
    else:
        raise ValueError(f'Unknown model type {model_type}')
    for W in mlp_inputs:
        dtype = W.weight.dtype
...
class QKRotationWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        k_c = args[1].clone().detach()  # k_c is the original k
...
def add_qk_rotation_wrapper_after_function_call_in_submodule(module, function_name, *args, **kwargs):
    '''
    This function adds a rotation wrapper after the output of a function call in forward. 
    Only calls directly in the forward function are affected. calls by other functions called in forward are not affected.
    '''
    ...
    attr_name = f"{function_name}_qk_rotation_wrapper"
    assert not hasattr(module, attr_name)
    wrapper = monkeypatch.add_wrapper_after_function_call_in_module(module,
                                                                    function_name, functools.partial(QKRotationWrapper, *args, **kwargs))
    setattr(module, attr_name, wrapper)
