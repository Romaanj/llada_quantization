...
def gptq_fwrd(model, dataloader, dev, args):
    ...
    if 'llada' in model.__class__.__name__.lower():
        ...
        model.model.transformer.ln_f = model.model.transformer.ln_f.to(dev)
    else:
    ...
    inps = torch.zeros(
        (args.nsamples, model.seqlen, model_utils.get_hidden_size(model)), dtype=dtype, device=dev
    )
    ...
    if 'llada' in model.__class__.__name__.lower():
        ...
        model.model.transformer.ln_f = model.model.transformer.ln_f.cpu()
    else:
    ...
    input_kwargs = {}
    if 'attention_mask' in cache and cache['attention_mask'] is not None:
        input_kwargs['attention_mask'] = cache['attention_mask']
    if 'position_ids' in cache and cache['position_ids'] is not None:
    ...
    if 'llada' in model.__class__.__name__.lower():
        sequential = [
                ['k_proj.module', 'v_proj.module', 'q_proj.module'],
                ['attn_out.module'],
                ['up_proj.module', 'ff_proj.module'],
                ['ff_out.module']
            ]
    else:
        sequential = [
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                if 'lm_head' in name:
                    ...
                    continue
                if args.int8_down_proj and ('down_proj' in name or 'ff_out' in name):
                    layer_weight_bits = 8
            ...
            for j in range(args.nsamples):
                outs[j] = layer(inps[j].unsqueeze(0), **input_kwargs)[0]
            for h in handles:
                h.remove()
...
def rtn_fwrd(model, dev, args):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for name in subset:
            layer_weight_bits = args.w_bits
            if 'lm_head' in name or 'ff_out' in name:
                layer_weight_bits = 16
                continue
            if args.int8_down_proj and ('down_proj' in name or 'ff_out' in name):
                layer_weight_bits = 8
