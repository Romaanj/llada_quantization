...
class QuantDreamAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None and isinstance(attention_mask, torch.Tensor):  # no matter the length, we just slice it
...
class QuantDreamDecoderLayer(nn.Module):
    def __init__(self, 
                 config: DreamConfig,
                 ori_layer,
                 args):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = QuantDreamAttention(
            org_module=ori_layer.self_attn,
            config=config,
            args=args,
            )
        self.mlp = QuantDreamMLP(
        ...
        self.input_layernorm = DuDreamRMSNorm(ori_layer.input_layernorm,eps=ori_layer.input_layernorm.variance_epsilon)
        self.post_attention_layernorm = DuDreamRMSNorm(ori_layer.post_attention_layernorm,eps=ori_layer.post_attention_layernorm.variance_epsilon)
    ...
    def register_duquant_params(self):        
        for name, module in self.named_modules():
            if isinstance(module, QuantDreamMLP) or isinstance(module, QuantDreamAttention):
